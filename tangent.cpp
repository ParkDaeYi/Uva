...
public:
	...
	int idx;
	bool ab;
	point(llt x = 0, llt y = 0, bool ab = false) : x(x), y(y), ab(ab), dx(0), dy(0), idx(0) {}
...
int main() {
	...
	while (t--) {
		cin >> n;
		for (int i = 0;i < n;++i) {
			cin >> Apos[i].x >> Apos[i].y; Apos[i].idx = i;
			pos[i].x = Apos[i].x, pos[i].y = Apos[i].y; pos[i].idx = i;
		}
		cin >> m;
		for (int i = 0;i < m;++i) {
			cin >> Bpos[i].x >> Bpos[i].y; Bpos[i].idx = i;
			pos[i + n].x = Bpos[i].x, pos[i + n].y = Bpos[i].y; pos[i + n].ab = 1; pos[i + n].idx = i;
		}
		for (int i = 1;i < n + m;++i) {
			if (pos[i] < pos[0]) swap(pos[i], pos[0]);
		}
		for (int i = 1;i < n + m;++i) pos[i].dx = pos[i].x - pos[0].x, pos[i].dy = pos[i].y - pos[0].y;
		sort(pos + 1, pos + n + m);
		for (int i = 0;i < n + m;++i) {
			while (convex.size() > 1 && ccw(convex[convex.size() - 2], convex.back(), pos[i]) <= 0) convex.pop_back();
		...
		for (int i = 0;i < sz;++i) {
			...
			if (convex[i].ab != convex[j].ab) {
				if (convex[i].ab) bp.push_back(convex[i]), ap.push_back(convex[j]);
				else ap.push_back(convex[i]), bp.push_back(convex[j]);
			}
		}
		for (int i = 0;i < n + m;++i) {
			if (pos[i].ab) {
		...
		double ans = 0;
		for (int i = 1;i < hull.size() - 1;++i) ans += ccw(hull[0], hull[i], hull[i + 1]);
		if (ap[0] == ap[1]) {
		...
		convex.clear(); hull.clear(); ap.clear(); bp.clear();
		fill(Apos, Apos + n, 0); fill(Bpos, Bpos + m, 0); fill(pos, pos + n + m, 0);
	}
	return 0;
}